...
const getWalletInfo = asyncHandler(async (req, res) => {
    ...
    const collectableSteps = stepsDoc ? (stepsDoc.stepsCount - (stepsDoc.convertedSteps || 0)) : 0;
    ...
    return res
        ...
        .json(new ApiResponse(200, {
            ...
            collectableSteps: collectableSteps
...
const convertSteps = asyncHandler(async (req, res) => {
    ...
    const collectableSteps = stepsDoc ? (stepsDoc.stepsCount - (stepsDoc.convertedSteps || 0)) : 0;
    if (!stepsDoc || collectableSteps < stepsToConvert) {
        throw new ApiError(400, `Insufficient steps. You have ${collectableSteps} steps available to convert.`)
    }
    ...
    // Increment convertedSteps and credit coins atomically
    ...
    return res
        ...
        .json(new ApiResponse(200, {
            coinsEarned,
            stepsConverted: stepsToConvert,
            newConvertedSteps: updatedSteps.convertedSteps,
            collectableSteps: updatedSteps.stepsCount - updatedSteps.convertedSteps,
...
const getStepsLeaderboard = asyncHandler(async (req, res) => {
    ...
    const enriched = steps.map(s => ({
        username: s.username,
        totalStepsWalked: s.totalStepsWalked,
...
export { getWalletInfo, convertSteps, convertOrbs, getLeaderBoard, getStepsLeaderboard, getBetsWonLeaderboard, getPortfolioLeaderboard }