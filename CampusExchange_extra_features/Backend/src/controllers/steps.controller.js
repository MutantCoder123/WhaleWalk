...
const updateSteps = asyncHandler(async (req, res) => {
    ...
    const steps = await Steps.findOneAndUpdate(
        { username },
        { $set: { stepsCount, totalStepsWalked: stepsCount } },
        { new: true }
    )
    ...
    const unlockedAchievements = await checkAndUnlockAchievements(username);

    return res
        .status(200)
        .json(new ApiResponse(200, { steps, unlockedAchievements }, "Steps updated successfully"))
})
