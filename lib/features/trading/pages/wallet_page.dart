...
class _WalletPageState extends ConsumerState<WalletPage> {
  ...
  Future<void> _convertNow() async {
    ...
    if (typedSteps == 0 && typedOrbs == 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text("Enter steps or orbs to convert"),
    ...
    try {
      ...
      if (typedSteps > 0) {
        final result =
            await ref.read(walletProvider.notifier).convertSteps(typedSteps);
        if (result) {
      ...
      if (typedOrbs > 0) {
        final result =
            await ref.read(walletProvider.notifier).convertOrbs(typedOrbs);
        if (result) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                "✅ Earned ${totalCoinsEarned.toStringAsFixed(2)} Campus Coins!"),
  ...
  Widget build(BuildContext context) {
    ...
    final availableSteps = wallet.availableSteps;
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  _buildBalanceMetric("COINS",
                      "${availableCoins.toStringAsFixed(2)}", Colors.amber),
                  _buildBalanceMetric("STEPS", "$availableSteps", Colors.white),
                  _buildBalanceMetric(
                      "ORBS", "$availableOrbs", Colors.purpleAccent),
                ],
              ),
            ),
              ...
              child: Column(
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      Text("Rate: 100 Steps = 1 Coin",
                          style: TextStyle(color: Colors.grey.shade400)),
                      Text("Avail: $availableSteps",
                          style: TextStyle(
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      focusedBorder: OutlineInputBorder(
                          borderSide:
                              const BorderSide(color: Color(0xFF00D09C)),
                          borderRadius: BorderRadius.circular(12)),
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text("Receive: ${_stepCoins.toStringAsFixed(2)} Coins",
                          style: const TextStyle(
                              color: Color(0xFF00D09C),
                              fontWeight: FontWeight.bold,
                              fontSize: 14)),
                      Text(
                          "Rem: ${remainingSteps >= 0 ? remainingSteps : 0} Steps",
                          style: TextStyle(
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text("Rate: 1 Orb = 5 Coins",
                          style: TextStyle(color: Colors.grey.shade400)),
                      Text("Avail: $availableOrbs",
                          style: TextStyle(
                              color: Colors.grey.shade400,
                              fontWeight: FontWeight.bold)),
                    ],
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text("Receive: ${_orbCoins.toStringAsFixed(2)} Coins",
                          style: const TextStyle(
                              color: Colors.purpleAccent,
                              fontWeight: FontWeight.bold,
                              fontSize: 14)),
                      Text(
                          "Rem: ${remainingOrbs >= 0 ? remainingOrbs : 0} Orbs",
                          style: TextStyle(
                              color: remainingOrbs >= 0
                                  ? Colors.white
                                  : Colors.red,
                              fontWeight: FontWeight.bold,
                              fontSize: 14)),
              ...
              data: (txns) {
                ...
                return Column(
                  children: txns.map((t) {
                    ...
                    final timePrefix = isToday
                        ? "Today,"
                        : "${t.createdAt.day}/${t.createdAt.month}";
                    final timeString =
                        "$timePrefix ${t.createdAt.hour.toString().padLeft(2, '0')}:${t.createdAt.minute.toString().padLeft(2, '0')}";

                    return _buildTxRow(
                        t.title,
                        "${t.isPositive ? '+' : ''}${t.amount.toStringAsFixed(2)} Coins",
                        timeString,
                        t.isPositive);
                  }).toList(),
                );
